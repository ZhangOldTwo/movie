package com.hw.movie.common;

public class Constants {

    public static final String APP_ID = "95jydTbNytTqKBBdMDGHK6MUKxXGBFJccJiyWBrJuRzN";
    public static final String SDK_KEY = "7jAFV6rvETuiDnXiFXSszdgJDDHHFat9vPZFRMnM2ure";

    /**
     * IR预览数据相对于RGB预览数据的横向偏移量，注意：是预览数据，一般的摄像头的预览数据都是 width > height
     */
    public static final int HORIZONTAL_OFFSET = 0;
    /**
     * IR预览数据相对于RGB预览数据的纵向偏移量，注意：是预览数据，一般的摄像头的预览数据都是 width > height
     */
    public static final int VERTICAL_OFFSET = 0;


    public static final String IS_LOCK = "LOCK_FLAG";
    public static final String IS_FACE = "IS_FACE";
    public static final String IS_HAPPY = "IS_HAPPY";
    public static final String LOCK_PASSWORD = "LOCK_PASSWORD";

}

